...
const FRONTEND_BASE_URL = (process.env.AGENTPOWERS_FRONTEND_URL || "https://agentpowers.ai").replace(/\/$/, "");
...
export async function handleGetSkillReviews(args) {
...
export async function handleStartCheckout(args) {
    const slug = args.slug != null ? String(args.slug) : "";
    if (!slug) {
        return "Missing required argument: slug";
    }
...
export async function handleGetAccountProfile() {
    ...
    const profile = await apiGet("/v1/users/profile", undefined, auth);
    const lines = [
...
export async function handleListPurchases(args) {
    const auth = loadAuthToken();
    if (!auth) {
        return "Not authenticated. Run `npx @agentpowers/cli login` first.";
    }
...
//# sourceMappingURL=handlers-commerce.js.map