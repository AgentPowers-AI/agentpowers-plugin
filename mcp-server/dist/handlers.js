...
import { readdirSync, rmSync, realpathSync } from "node:fs";
...
export async function handleSearchMarketplace(args) {
    const params = {};
    if (args.query)
...
export async function handleGetSkillDetails(args) {
...
export async function handleInstallSkill(args) {
    ...
    try {
        ...
        const download = await apiGet(`/v1/skills/${slug}/download`, undefined, auth);
        const type = detail.type === "agent" ? "agent" : "skill";
        const secStatus = detail.security_status ?? detail.ap_security_status ?? "pass";
        const result = await downloadAndExtract(download.url, slug, type, detail.source, detail.version, secStatus, targetTool);
        // Fire-and-forget installation tracking (never awaited for errors)
        void recordInstallation(slug, "mcp", detail.source ?? "agentpowers", hostname(), auth);
...
export async function handleCheckPurchaseStatus(args) {
    ...
    const status = await apiGet(`/v1/purchases/${encodeURIComponent(purchaseId)}/status`, undefined, auth);
    const lines = [
...
export async function handleUninstallSkill(args) {
    const slug = String(args.slug);
    if (!validateSlug(slug)) {
        return `Invalid slug "${slug}" — slugs must be lowercase alphanumeric with hyphens only.`;
    }
