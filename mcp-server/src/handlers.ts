...
export async function handleInstallSkill(
  ...
  if (detail.price_cents > 0 && !auth) {
    return `"${detail.title}" costs ${formatPrice(detail.price_cents)}. Run \`ap login\` first, then purchase it.`;
  }
  ...
  try {
    ...
    const installDir = await downloadAndExtract(
      download.url,
      slug,
      type,
      detail.source,
    ...
    return `Installed "${detail.title}" to ${installDir}`;
  } catch (error) {
...
export async function handleCheckPurchaseStatus(
  ...
  const purchaseId = String(args.purchase_id);
  const auth = loadAuthToken();

  if (!auth) {
    return "Not authenticated. Run `ap login` in your terminal first.";
  }
...
export async function handleCheckForUpdates(): Promise<string> {
  ...
  if (updates.length > 0) {
    ...
    lines.push("\nUse `ap update` in your terminal to install updates.");
  }
