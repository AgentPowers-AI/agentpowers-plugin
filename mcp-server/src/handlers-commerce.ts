...
export async function handleGetCategories(): Promise<string> {
  const data = await apiGet<CommerceCategoriesResponse>("/v1/categories");
  ...
  for (const cat of categories) {
    ...
    lines.push(`## ${icon}${cat.name} (\`${cat.slug}\`)`);
    if (cat.description) {
      lines.push(cat.description);
    }
    lines.push(`- **Skills:** ${cat.skill_count}`);
    lines.push("");
  }
...
export async function handleStartCheckout(
  ...
  const checkoutUrl = `https://agentpowers.ai/skills/${encodeURIComponent(slug)}?action=buy`;
