...
beforeEach(() => {
  ...
  vi.stubEnv("AGENTPOWERS_API_URL", "https://test-api.example.com");
});
...
describe("apiGet", () => {
  it("builds the correct URL with base and path", async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith(
      "https://test-api.example.com/v1/skills",
  ...
  it("falls back to AP_API_BASE when AGENTPOWERS_API_URL is not set", async () => {
    vi.unstubAllEnvs();
    vi.stubEnv("AP_API_BASE", "https://legacy-api.example.com");
    ...
    const calledUrl = mockFetch.mock.calls[0][0] as string;
    expect(calledUrl).toMatch(/^https:\/\/legacy-api\.example\.com/);
  });
...
describe("apiPost", () => {
  it("sends POST request with JSON body", async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith(
      "https://test-api.example.com/v1/checkout",
      expect.objectContaining({
        method: "POST",
...
describe("formatAPIError", () => {
  it("maps 401 to login message", () => {
    ...
    expect(formatAPIError(err)).toBe("Not logged in. Run `ap login` in your terminal first.");
  });
