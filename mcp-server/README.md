...
| `get_categories` | List all marketplace skill categories |
| `get_skill_reviews` | Fetch reviews for a specific skill |
| `start_checkout` | Begin the purchase flow for a paid skill (returns checkout URL) |
| `get_account_profile` | Retrieve the authenticated user's account profile |
| `list_purchases` | List all skills purchased by the authenticated user |
